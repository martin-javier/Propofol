...
smooth_age_df_1 <- data.frame(
  x = x_age_1,
  fit = exp(fitted_age_1),
  upper = exp(fitted_age_1 + 2 * se_age_1),
  lower = exp(fitted_age_1 - 2 * se_age_1)
)
...
ggplot(smooth_age_df_1, aes(x = x, y = fit)) +
  ...
  labs(title = "Estimated Effect: Age", x = "Age") +
...
ggplot(smooth_bmi_df_1, aes(x = x, y = fit)) +
  ...
  labs(title = "Estimated Effect: BMI", x = "BMI") +
  ylab(expression("Hazard Ratio " * exp(hat(beta)))) +
  theme.adjusted
...
ggplot(smooth_apache_df_1, aes(x = x, y = fit)) +
  ...
  labs(title = "Estimated Effect: ApacheIIScore", x = "ApacheIIScore") +
  ylab(expression("Hazard Ratio " * exp(hat(beta)))) +
  theme.adjusted
