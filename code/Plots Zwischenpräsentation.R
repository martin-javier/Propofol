...
ggplot(unique_patients, aes(x = ApacheIIScore)) +
...
# Anteil Verstorbene vs. Entlassene berechnen
outcome_counts <- unique_patients %>%
  summarise(
    Verstorben = sum(PatientDied, na.rm = TRUE),
    Entlassen = sum(PatientDischarged, na.rm = TRUE)
  ) %>%
...
ggplot(data_EK, aes(x = Age, y = ApacheIIScore)) +
